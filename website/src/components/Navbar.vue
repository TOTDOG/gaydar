...
import { ref } from "vue";
import { hasAdminPrivledges } from "../stores/adminState.ts";
...
<template>
    <span id="navbar" style="border-bottom: 1.5px solid black">
        ...
        <span
            ><p class="logo-text" :class="{ admin: hasAdminPrivledges }">
                TOTDOG
            </p>
            <p v-if="hasAdminPrivledges" class="admin-marker">Admin</p></span
        >
        ...
        <div tabindex="0" class="dropdown">
            ...
            <div class="dropdown-content">
                <router-link to="/form" class="droplink"
                    >Submit Prompts</router-link
                >
                <router-link to="/propose-attribute" class="droplink"
                    >Propose an Attribute</router-link
                >
                <router-link to="/vote-on-attributes" class="droplink"
                    >Vote on Attributes</router-link
                >
            </div>
        </div>
        <div class="spacer"></div>
        ...
        <div class="spacer"></div>
        <div tabindex="0" class="dropdown">
            <router-link to="/admin-login" class="dropbtn">Admin</router-link>
        </div>
        ...
        <Teleport to="body">
            ...
            <div class="mobile-menu" :class="{ open: menuOpen }">
                ...
                <div class="mobile-dropdown">
                    ...
                    >
                        Contribute
                        <span class="arrow" :class="{ rotated: contributeOpen }"
                            >▾</span
                        >
                    </div>
                    <div
                        class="mobile-submenu"
                        :class="{ open: contributeOpen }"
                    >
                        <router-link
                            to="/form"
                        ...
                        >
                        <router-link
                            to="/propose-attribute"
                            class="mobile-sublink"
                            @click="menuOpen = false"
                            >Propose an Attribute</router-link
                        >
                        <router-link
                            to="/vote-on-attributes"
                            class="mobile-sublink"
                            @click="menuOpen = false"
                            >Vote on Attributes</router-link
                        >
...
#navbar {
    ...
    background-color: rgba(74, 65, 60, 0.5);
