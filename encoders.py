...
class EncoderRegistry:
    def __init__(self, save_dir: str = 'encoder_weights'):
    ...
    def register_all(self, parameters: list[dict], temperature: float = 0.2):
        """
        Register encoders for all active parameters from the DB parameter list.
        Safe to call again when new parameters are added — skips existing ones.
        """
    ...
    def _add(self, attr_id: str, description: str,
        ...
        # Similarity-weighted drift correction so the new encoder starts
        # in the post-trained coordinate space, not the raw sentence encoder space
    ...
    def save(self):
        for attr_id, encoder in self.encoders.items():
            ...
            }, self.save_dir / f"{attr_id}.pt")
        print(f"Saved {len(self.encoders)} encoders to {self.save_dir}")
    ...
    def load_all(self):
        ...
        print(f"Loaded {len(self.encoders)} encoders from {self.save_dir}")